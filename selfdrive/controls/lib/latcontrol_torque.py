...
class LatControlTorque(LatControl):
  def __init__(self, CP, CI):
    ...
    # the closed loop characteristic equation (for eigenvalues of a) is
    # s^2 + k_p * s + k_i = 0
    # k_p has units of [s^(-1)]
    # k_i has units of [s^(-2)]
    # critical damping is k_i = (k_p/2)^2

    # ratio of tracking time constant to relaxation time
    # smaller means better performance, but more risk of instability
    self.tracking_time_ratio = 0.5 # dimensionless
